...
class ResNet(nn.Module):
    ...
    def __init__(self, block: BasicBlock, num_blocks: List[int],
                 num_classes: int, nf: int, feat_dim: int=128,
                 dist_linear=False) -> None:
        """
        ...
        if dist_linear:
            print('Using Prototype Network')
            self.linear = distLinear(nf * 8 * block.expansion, num_classes)
        else:
            self.linear = nn.Linear(nf * 8 * block.expansion, num_classes)

        dim_in = nf * 8 * block.expansion
...
def resnet18(nclasses: int, nf: int=64, dist_linear=False) -> ResNet:
    """
