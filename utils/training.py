...
def evaluate(model: ContinualModel, dataset: ContinualDataset, last=False, eval_ema=False, ema_model=None) -> Tuple[list, list]:
    ...
    #     elif ema_model == 'plastic_model_ema':
    #         print('setting evaluation model to plastic EMA model')
    #         curr_model = model.plastic_model_ema
    #     elif ema_model == 'stable_model_ema':
    #         print('setting evaluation model to Stable EMA model')
    #         curr_model = model.stable_model_ema
    ...
    for k, test_loader in enumerate(dataset.test_loaders):
        ...
        for data in test_loader:
            ...
            if 'active_dendrites' in model.NAME:
...
def train(model: ContinualModel, dataset: ContinualDataset,
    ...
    lst_ema_models = ['plastic_model', 'stable_model', 'ema_model', 'plastic_model_ema', 'stable_model_ema', 'net_before_mc']
    ema_loggers = {}
    ema_results = {}
    ...
    if model.NAME != 'icarl' and model.NAME != 'pnn' and not 'active_dendrites' in model.NAME and dataset.NAME != 'gcil-cifar100':
    ...
    for t in range(dataset.N_TASKS):
        ...
        for epoch in range(args.n_epochs):
            ...
            for i, data in enumerate(train_loader):
                ...
                elif hasattr(dataset.train_loader.dataset, 'is_noise'):
                    inputs, labels, not_aug_inputs, is_noise = data
                    inputs, labels = inputs.to(model.device), labels.to(
                        model.device)
                    not_aug_inputs = not_aug_inputs.to(model.device)
                    is_noise = is_noise.to(model.device)
                    model.is_noise = is_noise
